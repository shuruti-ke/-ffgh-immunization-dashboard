...
def parse_messy_date(date_val):
    ...
    formats = [
        "%Y-%m-%d",           # 2026-04-06
        "%d/%m/%Y %H:%M",     # 6/3/26 11:48 (will fail year, but try)
...
def process_data(uploaded_file):
    """Load and clean CHEW log data"""
    # Read Excel with flexible engine
    ...
    date_candidates = ['start', 'Start', 'date', 'Date', 'submission_date']
    ...
    # Standardize village names (case-insensitive)
...
def main():
    ...
    if not uploaded_file:
        ...
        - Vaccine columns: `Vax_BCG`, `Vax_Oral Polio Vaccine (OPV) 0`, etc.
        """)
        return
    ...
    if village_col in df.columns:
        ...
        st.sidebar.warning("Village column not found")
    ...
    # Vaccine selector
    if vax_cols:
        # Clean vaccine names for display
    ...
    # Zero-dose calculation (BCG + OPV0 not received)
    ...
    with col1:
        ...
        if selected_vax and village_col in df_f.columns and selected_vax in df_f.columns:
            # Calculate coverage per village
            ...
            if not vax_cov.empty:
                fig_bar = px.bar(
                    ...
                    labels={'coverage': 'Coverage %', village_col: 'Village'}
                )
    ...
    with col2:
        ...
        if selected_vax and "date" in df_f.columns and selected_vax in df_f.columns:
            df_f_temp = df_f.set_index("date").resample("ME").agg({selected_vax: ["sum", "count"]}).dropna(how="all")
            if not df_f_temp.empty:
                df_f_temp.columns = ["vaccinated", "total"]
                df_f_temp["coverage"] = (df_f_temp["vaccinated"] / df_f_temp["total"] * 100).round(1)
                
                fig_line = px.line(
                    df_f_temp.reset_index(), 
                    x="date", 
                    y="coverage",
                    markers=True,
                    title=f"{selected_label} Monthly Coverage Trend",
                    labels={'coverage': 'Coverage %', 'date': 'Month'}
                )
    ...
    if selected_vax and village_col in df_f.columns and selected_vax in df_f.columns:
        vax_cov = df_f.groupby(village_col)[selected_vax].agg(['sum', 'count']).reset_index()
        vax_cov['coverage'] = (vax_cov['sum'] / vax_cov['count'] * 100).round(1)
        vax_cov = vax_cov[vax_cov['count'] >= 3]  # Reliable samples only
        ...
        if not cold_spots.empty:
            ...
            for _, row in cold_spots.head(5).iterrows():
                st.markdown(f"- **{row[village_col]}**: {row['coverage']:.1f}% coverage ({int(row['count']-row['sum'])} unvaccinated children)")
        else:
    ...
    st.markdown("---")
    st.caption(
        "💡 *Tip: Use filters to focus on specific villages, CHEWs, or time periods. "
        "Export data to share with LGA officials or plan outreach sessions.*"
    )
